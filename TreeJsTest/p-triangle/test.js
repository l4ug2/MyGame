...
function addCubesCamera()
{
  ...
  for(let evolution = 0; evolution <= distView*5; evolution = evolution + size)
  {
     ...
     randomCubes(size, 2, direction_view, grassCube, earthCube, grassCamera);
     ...
     for(let revolution = size; revolution <= evolution; revolution = revolution + size)
     {
        angleVariant.forEach(angle => {
          ...
          randomCubes(size, 2, position_view, grassCube, earthCube, grassCamera);
        }
      );
    }
  }
...
function randomCubes(size, height, position, topElement, underElement, group)
{
  const random = Math.floor(Math.random() * height) + 1;          
  for (let index = 0; index < random; index++) 
  {

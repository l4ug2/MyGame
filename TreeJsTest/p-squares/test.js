...
function RotateVector3 (original)
{
  //:THREE.Vector3
  var response = new THREE.Vector3();
...
function addCubes() {
  ...
  var grassGeometry = new THREE.BoxGeometry(size, size, size);
  var grassCube = new THREE.Mesh(grassGeometry, grassMaterials);
  ...
  for (let evolution_x = 0; evolution_x <= count;  evolution_x = evolution_x+3) {
    for (let evolution_z = 0; evolution_z <= count; evolution_z = evolution_z+3) {       
      quaterVariant.forEach(quater => {
        splitVariant.forEach(split => {   
          var grass_x = evolution_x*quater.x + split.x + size/2;
          var grass_z = evolution_z*quater.z + split.z + size/2;

          var grass_y = evolution_y*quater.y + split.y;
          
          const grassClone = grassCube.clone(true); // quadran +x+z    
...
function rotateView(isClockwise)
{
  ...
  var response_view = RotateVector3(original_view, null, true); //rotate on new angle
